...
import type { SmartContract } from '@coinbase/coinbase-sdk';
...
export async function deployContract(): Promise<SmartContract> {
  ...
  try {
    ...
    console.log(
      `NFT contract deployed at address: ${multiToken.getContractAddress()}`,
    );
    ...
    console.log(
      `Deploy Transaction Hash: ${multiToken.getTransaction().getTransactionHash()}`,
    );
    console.log(
      `Deploy Transaction URL: ${multiToken.getTransaction().getTransactionLink()}`,
    );
